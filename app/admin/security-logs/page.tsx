    ...
    <div className="container mx-auto py-8 px-4 space-y-6">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap gap-4">
          <Select value={eventTypeFilter} onValueChange={(val) => { setEventTypeFilter(val); setCurrentPage(1); setPageCursors({ 1: null }) }}>
          ...
          <Select value={severityFilter} onValueChange={(val) => { setSeverityFilter(val); setCurrentPage(1); setPageCursors({ 1: null }) }}>
            <SelectTrigger className="w-[200px]"><SelectValue placeholder="Severity" /></SelectTrigger>
            <SelectContent>
          ...
          <Button variant="outline" size="sm" onClick={() => { setCurrentPage(1); setPageCursors({ 1: null }); refetchEvents(); refetchStats() }}>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={6} className="text-center py-8">보안 이벤트가 없습니다</TableCell></TableRow>
            ) : (
              events.map((event) => (
                <TableRow key={event.id}>
                  <TableCell className="font-mono text-xs">{formatDate(event.createdAt)}</TableCell>
                  <TableCell><Badge variant="outline">{EVENT_TYPE_LABELS[event.eventType] || event.eventType}</Badge></TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      {getSeverityIcon(event.severity)}
                      <Badge className={SEVERITY_COLORS[event.severity]}>{event.severity.toUpperCase()}</Badge>
                    </div>
                  </TableCell>
                  <TableCell>
                    {event.user ? (
                  ...
                  <TableCell className="font-mono text-xs">{event.requestMethod} {event.requestPath || '-'}</TableCell>
                  <TableCell>
                    {event.details && (
                      <details>
                        <summary className="cursor-pointer text-sm text-primary hover:underline">View Details</summary>
                        <pre className="mt-2 p-2 bg-muted rounded text-xs overflow-auto max-w-md">{JSON.stringify(event.details, null, 2)}</pre>
                      </details>
                    )}
                  </TableCell>
                </TableRow>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-muted-foreground">Page {currentPage} of {totalPages} ({totalCount} total events)</div>
